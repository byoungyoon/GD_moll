...
<div class="d-flex p-2 flex-row-reverse bg-dark text-white">
	<%
		...
		if(memberEmail == null){
		%>
			<a class="nav-link btn btn-outline-secondary" href="<%=request.getContextPath() %>/login/login.jsp">로그인</a>
		<%
		}
		else if(memberEmail.equals("Admin")){
			%>
				<a class="nav-link btn btn-outline-secondary" href="<%=request.getContextPath() %>/login/logoutAction.jsp">로그아웃</a>
